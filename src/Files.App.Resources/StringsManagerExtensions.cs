...
namespace Files.App.Resources
{
	...
	/// </summary>
	public static class StringsManagerExtensions
	{
		/// <summary>
		...
		/// </summary>
		private static readonly CultureInfo _locale = new(ResourceManager.Locale);
		...
		public static string GetLocalizedYAML(this string resourceKey, IReadOnlyDictionary<string, object?> pairs)
		{
			...
			{
				throw new Exception($"Formatter could not get a valid result value for: '{resourceKey}'");
			}
		...
		/// <returns>The formatted localized resource string.</returns>
		public static string GetLocalizedYAML(this string resourceKey, params IReadOnlyDictionary<string, object?>[] pairs)
		{
			var mergedPairs = pairs.SelectMany(dict => dict).ToDictionary(pair => pair.Key, pair => pair.Value);
			return resourceKey.GetLocalizedYAML(mergedPairs);
		}
		...
		/// <returns>The formatted localized resource string.</returns>
		public static string GetLocalizedYAML(this string resourceKey, params object[] values)
		{
		...
		public static string GetLocalizedYAML(this string resourceKey) => resourceKey.GetLocalizedYAML(0.ToFormatPairs(string.Empty));
	}
}
